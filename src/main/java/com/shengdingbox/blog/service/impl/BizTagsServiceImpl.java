...
import com.shengdingbox.blog.service.BizTagsService;
 ...
 */
@Service
public class BizTagsServiceImpl extends BizTagsService {
    ...
    public Tags insert(Tags entity) {
        ...
        if (this.getByName(entity.getName()) != null) {
            throw new ServiceException("标签添加失败，标签已存在！[" + entity.getName() + "]");
        }
    ...
    public boolean removeByPrimaryKey(Long primaryKey) {
        ...
        if (!CollectionUtils.isEmpty(articleTags)) {
            throw new ServiceException("当前标签下存在文章信息，禁止删除！");
        }
    ...
    public boolean updateSelective(Tags entity) {
        ...
        if (old != null && !old.getId().equals(entity.getId())) {
            throw new ServiceException("标签修改失败，标签已存在！[" + entity.getName() + "]");
        }
        entity.setUpdateTime(new Date());
