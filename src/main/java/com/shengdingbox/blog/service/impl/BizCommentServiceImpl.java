...
import com.shengdingbox.blog.entity.BizCommentDTO;
...
public class BizCommentServiceImpl implements BizCommentService {
    ...
    public void doSupport(Long id) {
        String key = IpUtil.getIpAddr(RequestHolder.getRequest()) + "_doSupport_" + id;
    ...
    public void doOppose(Long id) {
        String key = IpUtil.getIpAddr(RequestHolder.getRequest()) + "_doOppose_" + id;
        ValueOperations<String, Object> operations = redisTemplate.opsForValue();
        if (redisTemplate.hasKey(key)) {
