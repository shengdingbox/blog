...
import com.shengdingbox.blog.enums.CommentStatusEnum;
...
import com.shengdingbox.blog.service.BizArticleTagsService;
...
import com.zhouzifei.tool.exception.ArticleException;
...
public class BizArticleServiceImpl implements BizArticleService {
    ...
    public void doPraise(Long id) {
        ...
        love.setUserIp(IpUtil.getIpAddr(RequestHolder.getRequest()));
    ...
    public boolean publish(Article article, Long[] tags, MultipartFile file) {
        ...
        if (null != file) {
            ...
            VirtualFile virtualFile = uploader.upload(file, FileUploadType.COVER_IMAGE.getPath(), configs);
            // 保存封面图片
    ...
    public boolean updateTopOrRecommendedById(String type, Long id) {
        ...
        if ("top".equals(type)) {
            ...
            throw new ServiceException(ResponseStatus.INVALID_PARAMS.getMessage());
        }
